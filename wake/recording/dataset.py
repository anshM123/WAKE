...
def load_synchronized_session(session:str|Path,window_size:int=10)->tuple[np.ndarray,np.ndarray]:
    rows=load_synchronized_rows(session)
    motion=PoseMotionEstimator();base=np.asarray([instantaneous_features(row,motion.update(row.pose)) for row in rows]);targets=np.asarray([[*row.telemetry.accel_body_g,*row.telemetry.gyro_body] for row in rows]);return temporal_windows(base,targets,window_size)

def load_synchronized_rows(session:str|Path)->list[SynchronizedSample]:
    rows=[]
